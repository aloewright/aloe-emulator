...
let analysisBuffer = "";
const MAX_ANALYSIS_BUFFER_SIZE = 5000;

const flushAnalysis = () => {
  if (aiWorker && analysisBuffer) {
    try {
      aiWorker.postMessage({ type: "analyze", data: analysisBuffer });
      analysisBuffer = "";
    } catch (err) {
...
const debouncedFlush = debounce(flushAnalysis, 300);
  ...
  if (term) {
    try {
      ...
      if (aiStore.inlineEnabled && aiWorker) {
        ...
        if (analysisBuffer.length > MAX_ANALYSIS_BUFFER_SIZE) {
