  ...
  if (term) {
    try {
      ...
      if (aiStore.inlineEnabled && aiWorker) {
        if (aiBuffer.length < MAX_AI_BUFFER_SIZE) {
          aiBuffer += data;
        } else {
          // If buffer is full, we might be in a very fast output stream.
          // We'll just keep the end of the stream to ensure we capture the latest context
          const overage = aiBuffer.length + data.length - MAX_AI_BUFFER_SIZE;
          if (overage > 0) {
            // Keep the end of the existing buffer + new data
            // effectively shifting the window
            aiBuffer = aiBuffer.slice(overage) + data;
          } else {
            aiBuffer += data;
          }
        }
...
let aiBuffer: string = "";
const MAX_AI_BUFFER_SIZE = 50 * 1024; // 50KB

const debouncedAnalyze = debounce(() => {
  if (aiWorker && aiBuffer.length > 0) {
    try {
      aiWorker.postMessage({ type: "analyze", data: aiBuffer });
      aiBuffer = "";
    } catch (err) {
