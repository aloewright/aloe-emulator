...
// AI Analysis Buffer Management
let aiAnalysisBuffer = "";
const AI_ANALYSIS_BUFFER_LIMIT = 5000;

const flushAiAnalysis = () => {
  if (!aiAnalysisBuffer || !aiWorker) return;
...
const debouncedFlush = debounce(flushAiAnalysis, 300);

const queueAiAnalysis = (data: string) => {
  aiAnalysisBuffer += data;

  if (aiAnalysisBuffer.length >= AI_ANALYSIS_BUFFER_LIMIT) {
