  ...
  if (term) {
    try {
      ...
      if (aiStore.inlineEnabled && aiWorker) {
        ...
        // Safety cap: if buffer gets too large (e.g., massive output), flush immediately
        // to prevent memory issues and ensure the worker gets data eventually.
        if (aiAnalysisBuffer.length > 5000) {
          flushAiAnalysis.cancel(); // Cancel any pending debounce

          try {
            aiWorker.postMessage({ type: "analyze", data: aiAnalysisBuffer });
          } catch (err) {
            console.error(
              "[Terminal] Failed to post message to AI Worker:",
              err,
            );
          }
