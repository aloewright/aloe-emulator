<template>
  ...
  >
    <div class="flex flex-col h-[50vh] max-h-[500px]">
      <!-- Header / Input Area -->
      <div class="flex items-center px-4 py-3 border-b border-gray-700 bg-gray-900/50 justify-between">
        <div class="flex items-center flex-1 cursor-text" @click="focusInput">
             ...
             />
             <span v-else class="text-lg font-semibold text-gray-200">AI Settings</span>
        </div>
      ...
      <!-- Main Content -->
      <div v-if="!showSettings" class="flex-1 overflow-y-auto p-4 bg-gray-900/30">
        <div v-if="aiStore.models.length === 0" class="text-red-400 mb-2 text-sm">
           No models found. Check Settings or ensure Ollama is running.
        </div>
        
        <div v-if="aiStore.error" class="p-3 bg-red-900/20 text-red-200 rounded border border-red-800 mb-4 text-sm">
          Error: {{ aiStore.error }}
        </div>
      ...
      <!-- Settings View -->
      <div v-else class="flex-1 overflow-y-auto p-6 bg-gray-900/30 text-gray-300">
          <div class="space-y-6">
              <!-- Provider Selection -->
              <div class="space-y-2">
                  <label class="block text-sm font-medium text-gray-400">AI Provider</label>
                  <div class="flex gap-4">
                      <label class="flex items-center gap-2 cursor-pointer">
                          <input type="radio" value="ollama" v-model="selectedProvider" class="accent-purple-500" @change="saveSettings">
                          <span>Ollama (Local)</span>
                      </label>
                      <label class="flex items-center gap-2 cursor-pointer">
                          <input type="radio" value="openrouter" v-model="selectedProvider" class="accent-purple-500" @change="saveSettings">
              ...
              <div v-if="selectedProvider === 'openrouter'" class="space-y-2">
                  <label class="block text-sm font-medium text-gray-400">OpenRouter API Key</label>
                  <input 
                    v-model="apiKey"
                    type="password" 
                  ...
                  <p class="text-xs text-gray-500">
                      Keys are stored locally. Get a key at <a href="https://openrouter.ai" target="_blank" class="text-purple-400 hover:underline">openrouter.ai</a>
                  </p>
              </div>
              ...
              <!-- Model Refresh -->
               <div class="pt-4 border-t border-gray-700">
                  ...
                  >
                     <RefreshCw class="w-3 h-3" :class="{ 'animate-spin': loadingModels }" />
                     Refresh Models
                  </button>
               </div>
          </div>
      ...
      <!-- Footer -->
       <div v-if="!showSettings" class="px-4 py-2 border-t border-gray-700 text-xs text-gray-500 flex justify-between items-center bg-gray-900">
        <div class="flex items-center gap-2">
           <span>Model:</span>
           <select v-model="aiStore.activeModel" class="bg-gray-800 border-none rounded text-xs text-gray-300 max-w-[200px]">
...
import { Sparkles, Copy, Terminal, Check, Settings, RefreshCw } from "lucide-vue-next";
...
watch(() => aiStore.activeProvider, (val) => selectedProvider.value = val);
...
async function runCommand(cmd: string) {
    ...
    if (!panel) {
         ...
         console.error("Run Command Failed: Active panel not found", workspaceStore.panelLayout);
         return;
    }
    ...
    const terminalInstance = workspaceStore.terminals.find(t => t.id === frontendTerminalId);
    
    if (!terminalInstance || !terminalInstance.backendTerminalId) {
        aiStore.error = `Terminal not ready (Frontend ID: ${frontendTerminalId})`;
        console.error("Run Command Failed: No backend ID for terminal", terminalInstance);
        return;
    }
