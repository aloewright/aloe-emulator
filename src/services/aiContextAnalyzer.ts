...
export interface OutputPattern {
  ...
  extractor: (match: RegExpMatchArray, fullOutput: string) => Partial<TerminalContext>;
...
const SERVER_PATTERNS: OutputPattern[] = [
  ...
  {
    pattern: /(?:Server|server|App|app)?\s*(?:ready|running|listening|started)\s*(?:at|on)?\s*(https?:\/\/[^\s]+|localhost:\d+)/i,
    type: 'server',
    extractor: (match) => ({
      type: 'server',
    ...
    suggestionGenerator: (ctx) => ({
      ...
      dismissable: true,
      actions: [
        { id: 'logs', label: 'Open logs', icon: 'terminal', type: 'command', command: 'tail -f logs/*.log' },
        { id: 'routes', label: 'View routes', icon: 'lightbulb', type: 'info' },
        { id: 'tests', label: 'Run tests', icon: 'refresh-cw', type: 'command', command: 'npm test' },
        { id: 'optimize', label: 'Optimize code', icon: 'sparkles', type: 'navigate' },
      ],
    }),
  },
  // Express.js specific
...
const ERROR_PATTERNS: OutputPattern[] = [
  {
    ...
    suggestionGenerator: (ctx) => ({
      ...
      dismissable: true,
      actions: [
        { id: 'fix', label: 'Ask AI to fix', icon: 'sparkles', type: 'navigate' },
        { id: 'search', label: 'Search docs', icon: 'search', type: 'navigate' },
        { id: 'retry', label: 'Retry command', icon: 'refresh-cw', type: 'command' },
      ],
    }),
  },
  {
    ...
    suggestionGenerator: (ctx) => ({
      ...
      dismissable: true,
      actions: [
        { id: 'fix', label: 'Suggest installation', icon: 'download', type: 'navigate' },
  ...
  },
  {
    ...
    suggestionGenerator: (ctx) => ({
      ...
      dismissable: true,
      actions: [
        { id: 'ls', label: 'List files', icon: 'list', type: 'command', command: 'ls -la' },
        { id: 'create', label: 'Create file', icon: 'plus-square', type: 'navigate' },
      ],
    }),
  }
];
...
const GIT_PATTERNS: OutputPattern[] = [
  {
    ...
    suggestionGenerator: () => ({
      ...
      dismissable: true,
      actions: [
        { id: 'push', label: 'Push changes', icon: 'upload', type: 'command', command: 'git push' },
        { id: 'pull', label: 'Pull changes', icon: 'download', type: 'command', command: 'git pull' },
        { id: 'status', label: 'View status', icon: 'git-branch', type: 'command', command: 'git status' },
      ],
    }),
  },
];
...
export class AIContextAnalyzer {
  ...
  analyzeOutput(output: string): AISuggestion | null {
    ...
    // Check all patterns
    const allPatterns = [...SERVER_PATTERNS, ...ERROR_PATTERNS, ...GIT_PATTERNS];
    ...
    for (const patternDef of allPatterns) {
      ...
      if (match) {
        ...
        const suggestion = patternDef.suggestionGenerator(context as TerminalContext);
        if (suggestion) {
