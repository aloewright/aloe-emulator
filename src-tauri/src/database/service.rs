...
impl DatabaseService {
    ...
    pub async fn new(config: DatabaseServiceConfig) -> DatabaseResult<Self> {
        ...
        // Auto-unlock with Default Password (Attempt only)
        if let Some(entry) = local_db
            .get_master_password_entry(&current_device.device_id)
            .await?
        {
            // Only attempt auto-unlock with default password if it's likely still default
            // We do NOT reset the database if this fails, to prevent data loss for users with custom passwords
            let req = VerifyMasterPasswordRequest {
                password: "aloe".to_string(),
                device_id: None,
            };

            // Attempt verify - ignore errors, just leave locked if fails
            let _ = master_password_manager
